...
pub async fn main_thread<OpType: OpTrait+Serialize+DeserializeOwned>(callback: Arc<dyn Callback<OpType>>) {
    ...
    let throughput: f64 = arg_matches.value_of("throughput").unwrap().parse::<f64>().expect("Failed to parse command line argument");
    ...
    info!("Ledger throughput set to {}tx/s and latency set to {}ms", throughput, latency);
